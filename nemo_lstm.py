...
class NemoEnv(PipelineEnv):
    ...
    def rewards(self, state, data, action, contact):
        ...
        reward_dict["periodic"] = period_rew * 1.0
        ...
        reward_dict["feet_zd"] = feet_zd_rew * 0.1
    ...
    def periodicReward(self, info, data1, data0):
        ...
        l_spd_rew = 1 - jnp.exp(-1 * jnp.sum(lv**2) / (0.3 * SIGMA_FAC))
        r_spd_rew = 1 - jnp.exp(-1 * jnp.sum(rv**2) / (0.3 * SIGMA_FAC))
