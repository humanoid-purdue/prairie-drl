...
train_fn = functools.partial(
      ppo.train, num_timesteps=200000000, num_evals=40, episode_length = 2000,
       normalize_observations=True, unroll_length=20, num_minibatches=64,
      num_updates_per_batch=4, discounting=0.98, learning_rate=1.0e-4,
...
def progress(num_steps, metrics):
    ...
    plt.title('{}'.format(metrics['eval/episode_footplan_reward']))
    for key in y_data.keys():
