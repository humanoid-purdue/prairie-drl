...
from playground.base import BaseEnv
...
class Joystick(BaseEnv):
  ...
  def _post_init(self) -> None:
    ...
    self._torso_body_id = self._mj_model.body(self.consts.ROOT_BODY).id
    ...
    self._feet_site_id = np.array(
        [self._mj_model.site(name).id for name in self.consts.FEET_SITES]
    )
    self._hands_site_id = np.array(
        [self._mj_model.site(name).id for name in self.consts.HAND_SITES]
    )
    self._floor_geom_id = self._mj_model.geom("floor").id
    self._feet_geom_id = np.array(
        [self._mj_model.geom(name).id for name in self.consts.FEET_GEOMS]
    )

    foot_linvel_sensor_adr = []
    for site in self.consts.FEET_SITES:
